...
# run: ./extract_schema_data_to_csv.sh
#
#set -x # debuging
...
echo "$($TIMESTAMP), START remove old csv data files of schema: $SCRIPT_FILE" | tee -a ${LOG}
rm ${CSV_PATH}/${DB_SCHEMA}.*.csv*
echo "$($TIMESTAMP), START generate extract schema SQL: $SCRIPT_FILE" | tee -a ${LOG}
cat <<EOF_SQL | psql -X -A -w -t -U "${DB_USER}" -d "${DB_NAME}" ${DB_HOST} > "${SQLFILE}" 2>${LOG}
select 
...
# split csv files to 100MB chunks and remove original csv file. 60GB  ~ 30min on pg13-ware-test
for csv_file in $(ls ${CSV_PATH}/*.csv); do split --suffix-length=5 --line-bytes=100M "${csv_file}" "${csv_file}."; rm "${csv_file}"; done
echo "$($TIMESTAMP), END split csv files to 100M chunks: $SCRIPT_FILE" | tee -a ${LOG}
